...
def train_markov_model(corpus_file, state_size=3):
    ...
    return markovify.NewlineText(text, state_size=state_size, well_formed=True)
...
def generate_text(sentence_count=5, min_length=8, max_length=50, max_chars=500):
    """生成指定数量的随机文本"""
    results = []
    total_chars = 0
    ...
    for _ in range(sentence_count):
        sentence = markov_model.make_sentence(min_words=min_length, max_words=max_length)
        if sentence:
...
if __name__ == "__main__":
    train(combined=True)
    print(generate_text(max_chars=5000))
