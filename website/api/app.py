...
def generate_sentence(min_length=5, max_length=20):
    ...
    for _ in range(sentence_length):
        if not sentence:
            ...
            continue_length > 5
            and random.random() < continue_length / 24
            and len(sentence) < sentence_length - 3
        ):
            # 生成逗号
            comma = random.choices(commas, weights=[0.6, 0.05, 0.1, 0.05, 0.2])[0]
            sentence += comma
            continue_length = 0
            ...
            # 避免重复字符
            while next_char[0] == sentence[-1] or len(sentence) + len(
                next_char
            ) > sentence_length - (not no_dot):
                next_char = random.choices(chars, weights=probs)[0]
            sentence += next_char
...
def generate_paragraph(num_sentences_range=[4, 8], max_sentence_length=20):
    ...
    return "".join(
        generate_sentence(max_length=max_sentence_length) for _ in range(num_sentences)
    )


def generate_text(
    num_paragraphs_range=[3, 5], num_sentences_range=[4, 8], max_sentence_length=6
):
    num_paragraphs = random.randint(*num_paragraphs_range)
    return "\n".join(
        generate_paragraph(num_sentences_range, max_sentence_length)
...
def generate_chars(length):
    ...
    generated_text = "".join(
        generate_sentence(min_length=num_sentence, max_length=num_sentence)
        for num_sentence in arr_num_sentences
    )
    return jsonify({"loremText": generated_text})
